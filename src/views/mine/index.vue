<template>
  <div>
    ...
    <van-cell-group class="user-info" v-if="$store.state.user">
      ...
      >
        <div slot="title">
          <img class="avatar" :src="userMsgList.photo" alt="">
          <span class="title">{{ userMsgList.name }}</span>
        </div>
      </van-cell>
      <van-grid class="data-info" :border="false">
        <van-grid-item>
          <span class="count">{{ userMsgList.art_count }}</span>
          <span class="text">头条</span>
        </van-grid-item>
        <van-grid-item>
          <span class="count">{{ userMsgList.follow_count }}</span>
          <span class="text">关注</span>
        </van-grid-item>
        <van-grid-item>
          <span class="count">{{ userMsgList.fans_count }}</span>
          <span class="text">粉丝</span>
        </van-grid-item>
        <van-grid-item>
          <span class="count">{{ userMsgList.like_count }}</span>
...
export default {
  ...
  created () {
    if (this.$store.state.user) {
      this.onLoadUserMsgList() // 初始化加载
    }
  },

  methods: {
    async  onLoadUserMsgList () {
      const { data } = await getUserMsg()
      this.userMsgList = data.data
    }
  }
}
</script>
